...
const checkRemindersDueTomorrow = async () => {
  try {
    ...
    console.log(
      `📧 Found ${reminders.length} reminder(s) due tomorrow. Sending emails...`,
    );
    ...
    for (const reminder of reminders) {
      try {
        ...
        if (emailSent) {
          ...
          failCount++;
          console.error(
            `❌ Failed to send email for reminder ID: ${reminder.reminder_id}`,
          );
        }
      } catch (error) {
        failCount++;
        console.error(
          `❌ Error processing reminder ID: ${reminder.reminder_id}`,
          error,
        );
      }
    }
    ...
    console.log(
      `📊 Reminder check completed: ${successCount} emails sent, ${failCount} failed.`,
    );
  } catch (error) {
